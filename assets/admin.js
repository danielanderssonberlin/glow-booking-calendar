(function($){
  ...
  $(document).on('click', '.glowbc-accept-request', function(e){
    ...
    if (!confirm('Sind Sie sicher, dass Sie diese Anfrage annehmen möchten?')) {
      return;
    }
    $btn.prop('disabled', true).text('Akzeptiere...');
    $.post(GlowBC.ajaxUrl, {
  ...
  $(document).on('click', '.glowbc-delete-request', function(e){
    ...
    const $btn = $(this);
    const requestId = $btn.data('id');
    if (!confirm('Sind Sie sicher, dass Sie diese Anfrage ablehnen/löschen möchten?')) {
      return;
    }
    $btn.prop('disabled', true).text('Lösche...');
    $.post(GlowBC.ajaxUrl, {
